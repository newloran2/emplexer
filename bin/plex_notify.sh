...
echo "plex notify iniciado key=$key sleep_time=$sleep_time plex_base_url=$plex_base_url time_to_stop=$time_to_stop playback_duration=$playback_duration compare_duration=$compare_duration playback_position=$playback_position" > /tmp/emplexer_run_position.txt


while [[ 1 ]]; do
	echo "" > /tmp/emplexer_run_position.txt
	...
	if [[ $((playback_duration - playback_position)) -le $time_to_stop ]]; then
		...
		echo "foi menor $playback_position $playback_duration "  >> /tmp/emplexer_run_position.txt
	fi
	...
	if [[ $playback_position -le 0  || $last_playback_position -ge $compare_duration ]]; then
		...
		echo "$plex_base_url:/progress?key=$key&identifier=com.plexapp.plugins.library&time=$playback_position&state=stopped" >> /tmp/emplexer_run_position.txt
    ...
    echo "$plex_base_url:/progress?key=$key&identifier=com.plexapp.plugins.library&time=$playback_position&state=playing" >> /tmp/emplexer_run_position.txt
    if [[ $playback_position -gt 0  ]]; then
    	last_playback_position=$playback_position
    	echo "last_playback_position = $last_playback_position" >> /tmp/emplexer_run_position.txt
    fi    
	sleep $sleep_time
done
